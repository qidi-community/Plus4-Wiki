...
class BoxExtras:
    def __init__(self, config):
        ...
        self.extrude_gcode = "BOX_EXTRUDE HOTEND={hotendtemp}"

        # self.move_heating_gcode = "DISABLE_ALL_SENSOR\nMOVE_TO_TRASH\nM109 S{hotendtemp}"
        self.move_heating_gcode = "BOX_MOVE_HEATING HOTEND={hotendtemp}"
    ...
    def cmd_CUT_FILAMENT(self, gcmd):
        ...
        if slot_state != 0:
            ...
            if had_load_extruder != 'slot-1' and self.printer.lookup_object('hall_filament_width_sensor').diameter > 0.5:
                stepper = self.printer.lookup_object('box_stepper ' + str(had_load_extruder))
                stepper.dwell(0.5)
    ...
    def cmd_RETRY(self, gcmd):
        ...
        if step_name == 'QDE_004_001':
            ...
            had_load_extruder = self.get_value_by_key('last_load_slot','slot-1')
            temp_stepper = self.printer.lookup_object('box_stepper ' + had_load_extruder)
    ...
    def button_extruder_load(self, gcmd):
        ...
        self.gcode.run_script_from_command("BOX_HOME_XY")
        max_temp = 240
        if self.b_endstop_state:
            self.gcode.run_script_from_command("BOX_MOVE_HEATING HOTEND=%s" %(str(curt_filament_max_temp)))
    ...
    def button_extruder_unload(self, gcmd):
        ...
        if not self.b_endstop_state:
            self.gcode.run_script_from_command("BOX_MOVE_HEATING HOTEND=%" + str(curt_filament_max_temp) + "\nCUT_FILAMENT\nMOVE_TO_TRASH\nM83")
    ...
    def button_box_unload(self, gcmd):
        ...
        if self.printer.lookup_object('hall_filament_width_sensor').diameter > 0.5:
            ...
            if had_load_extruder == stepper.stepper_name:
